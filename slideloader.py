...
class SlideLoader(QObject):
    ...
    def set_zoom_stack(self):
        ...
        """
        #diff_center = np.abs(self.old_center - center_low_lvl)
        #diff_reserve = np.abs((self.slide_size[0] - self.slide_size[-1]) / 2)

        #if diff_center[0] > diff_reserve[0] or diff_center[1] > diff_reserve[1] or not self.new_file:  # check if new position will fit into current slides
        #    pass
    ...
    def get_zoom_stack(self, level: int = None):
        with QMutexLocker(self._stack_mutex):
            if level:
                return self.zoom_stack[level].copy()
            else:
                return self.zoom_stack.copy()
